...
-- Wrote down these instances explicitly below, but now with thunks outside *)
    ...
    (let fix arb_aux (size : nat) : G mynat :=
       ...
       | S size' =>
           freq [(1, thunkGen (fun _ => returnGen OO));
                 (1, thunkGen (fun _ => bindGen (arb_aux size') (fun p0 : mynat => returnGen (S1 p0))));
                 (1, thunkGen (fun _ => bindGen (arb_aux size') (fun p0 : mynat => returnGen (S2 p0))));
                 (1, thunkGen (fun _ => bindGen (arb_aux size') (fun p0 : mynat => returnGen (S3 p0))))]
       end in
     arb_aux) s
|}.
